...
class GithubRepositoryImpl @Inject constructor(
    ...
    private val redirectUri = "shengj://callback"
    ...
    override suspend fun searchRepos(query: String, page: Int, perPage: Int): Flow<Result<List<Repo>>> = flow {
        try {
    ...
    override suspend fun getUserRepos(page: Int, perPage: Int): Flow<Result<List<Repo>>> = flow {
        val token = getToken()
        if (token == null) {
            emit(Result.failure(Exception("Not authenticated")))
            return@flow
    ...
    override suspend fun getIssues(owner: String, repoName: String, page: Int, perPage: Int): List<Issue> {
    ...
    override suspend fun getIssueDetail(owner: String, repoName: String, issueNumber: Int): Issue {
        return withContext(Dispatchers.IO) {
